...
echo "🔻 Removendo Aplicações (02-apps)..."
kubectl delete -f "${K8S_DIR}/02-apps/" --ignore-not-found
...
# 3. Remover Caddy e IngressClass
echo "🔻 Removendo Caddy e Core (00-core)..."
helm uninstall caddy -n caddy-system 2>/dev/null || true
kubectl delete -f "${K8S_DIR}/00-core/" --ignore-not-found

# 4. Remover Namespaces Restantes
echo "🔻 Limpando Namespaces..."
kubectl delete namespace media caddy-system --ignore-not-found
