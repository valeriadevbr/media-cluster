...
apply_with_subst "${K8S_PATH}/media/00-core/"
apply_with_subst "${K8S_PATH}/media/01-storage/"

echo "🌐 Aplicando Ingress (${MEDIA_CLUSTER_NAME})..."
apply_with_subst "${K8S_PATH}/media/02-ingress/"
...
if [ "${MEDIA_SERVERS_IN_CLUSTER}" = "true" ]; then
  ...
  apply_k8s_file "${K8S_PATH}/media/03-apps/02-plex-internal.conditional.yaml"
else
  echo "🔗 Aplicando Plex (External/Shim)..."
  apply_k8s_file "${K8S_PATH}/media/03-apps/02-plex-external.conditional.yaml"
fi

if [ "${MEDIA_SERVERS_IN_CLUSTER}" = "true" ]; then
  echo "🎬 Aplicando Emby (Internal Cluster)..."
  apply_k8s_file "${K8S_PATH}/media/03-apps/02-emby-internal.conditional.yaml"
else
  echo "🔗 Aplicando Emby (External/Shim)..."
  apply_k8s_file "${K8S_PATH}/media/03-apps/02-emby-external.conditional.yaml"
fi

apply_with_subst "${K8S_PATH}/media/03-apps/"
